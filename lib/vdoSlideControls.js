const vdoControls = {
  ...
  init: async (reveal) => {    
    ...
    window.addEventListener("startVDO", async (event) => {
      console.log("starting VDO plugin")
    //reveal.on( 'ready', async (event) => {
      ...
      else{console.log("vdo event",event)}
    ...
    async function openConnection(){
      ...
      let FULL_URL = `https://vdo.ninja/?label=Host&datamode&push=${VDO_ID}&pw=${VDO_PW}&mv=1`;
      
      // Embedding VDO.Ninja iframe for sending commands

      document.getElementById("slide 0").innerHTML =`<iframe id="vdoIframe" 
        src=${FULL_URL} name="vdoIframe" style="width:100%; height:500px" ></iframe>`;
